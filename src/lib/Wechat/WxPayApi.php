...
use Dns192\Payment\lib\Wechat\implement\WxPayBizPayUrl;
use Dns192\Payment\lib\Wechat\implement\WxPayDownloadBill;
use Dns192\Payment\lib\Wechat\implement\WxPayMicroPay;
use Dns192\Payment\lib\Wechat\implement\WxPayRefundQuery;
use Dns192\Payment\lib\Wechat\implement\WxPayReport;
use Dns192\Payment\lib\Wechat\implement\WxPayResults;
use Dns192\Payment\lib\Wechat\implement\WxPayReverse;
use Dns192\Payment\lib\Wechat\implement\WxPayShortUrl;
use Dns192\Payment\lib\Wechat\implement\WxPayUnifiedOrder;
/**
 * 
...
class WxPayApi
{
    ...
    public static function unifiedOrder(WxPayUnifiedOrder $inputObj, $timeOut = 6)
    {
        ...
        if(!$inputObj->IsNotify_urlSet()){
            $inputObj->SetNotify_url(WxPayConfig::$notifyUrl);//异步通知url
        }
        
        $inputObj->SetAppid(WxPayConfig::$appId);//公众账号ID
        $inputObj->SetMch_id(WxPayConfig::$mchId);//商户号
        if (WxPayConfig::$sub_mch_id){
            $inputObj->SetSub_mch_id(WxPayConfig::$sub_mch_id);
            WxPayConfig::$sub_appId && $inputObj->SetSub_appid(WxPayConfig::$sub_appId);
        }
     ...
     * @return array 成功时返回，其他抛异常
     */
    public static function report(WxPayReport $inputObj, $timeOut = 1)
    {

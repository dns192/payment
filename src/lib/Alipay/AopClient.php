...
class AopClient {
    ...
    protected function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt) {
        ...
        throw new Exception(json_encode($logData,JSON_UNESCAPED_UNICODE));
    }

    /**
     * 生成用于调用收银台SDK的字符串
     * @param AlipayBase $request SDK接口的请求参数对象
     * @return string 
     * @author guofa.tgf
     */
    ...
    */
    ...
    /** 生成提交表单字符串
     * @param AlipayBase    $request    提交对象
     * @param string        $httpmethod 提交方法
     *
     * @return string
     * @throws Exception
     ...
     * @param string $authToken         针对用户授权接口，获取用户相关数据时，用于标识用户授权关系
     * @param string $appInfoAuthToken  第三方应用授权
     *
    ...
    public function checkSignAndDecrypt($params, $rsaPublicKeyPem, $rsaPrivateKeyPem, $isCheckSign, $isDecrypt) {
        ...
        if ($isDecrypt) {
            return $this->rsaDecrypt($bizContent, $rsaPrivateKeyPem);
        }

        return $bizContent;
    }

    public function encryptAndSign($bizContent, $rsaPublicKeyPem, $charset, $isEncrypt, $isSign) {
        // 加密，并签名
        if ($isEncrypt && $isSign) {
     ...
     * @param array|SimpleXMLElement $respObject
     * @param string $format
     *
     * @return string
     */
    ...
    /**
     * @param AlipayBase $request
     * @param SimpleXMLElement $responseContent
     *
     * @return SignData
     */
